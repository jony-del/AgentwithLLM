...
`LCPAdapter` ([agent_core/tools/adapters.py](agent_core/tools/adapters.py)) and `MultiAgentCoordinator` ([agent_core/agents/multi.py](agent_core/agents/multi.py)) are intentionally minimal stubs (`list_tools()` returns `[]`). They mark where local/custom-protocol tool integration and multi-agent coordination are meant to grow. (`MCPAdapter` is no longer a stub — it now lives in [agent_core/mcp/](agent_core/mcp/); see the Tools seam above.)
