...
| **session / transcript** | `bootstrap/state.js`（sessionId、全局状态）、`history.ts`（命令历史、paste/image ref、`/resume` 持久化）、session ingress | `session.py`（**仅 per-run** SessionContext：todos、workspace、subagent factory、depth）。**无跨会话 transcript 持久化/resume** | **差异大** |
