...
def test_memory_enabled_extracts_after_run(tmp_path: Path) -> None:
    agent = _agent(tmp_path, memory=True)
    agent.run("I prefer dark mode and Python 3.11")
...
def test_memory_enabled_recalls_relevant_memory(tmp_path: Path) -> None:
    agent = _agent(tmp_path, memory=True)
    agent.run("I prefer dark mode and Python 3.11")  # seeds the store

    result = agent.run("remind me about my dark mode preference")
...
def test_memory_disabled_is_a_no_op(tmp_path: Path) -> None:
    agent = _agent(tmp_path, memory=False)
    result = agent.run("I prefer dark mode and Python 3.11")
